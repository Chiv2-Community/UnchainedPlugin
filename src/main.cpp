...
#include "../include/ownershipOverrides.h"
#include "assetLoading.h"
#include "../include/unchainedIntegration.h"
#include "../include/adminControl.h"
#include "etcHooks.h"
#include "../include/FunctionHookEnabler.hpp"
...
void handleRCON() {
	...
	LOG_INFO(L"[RCON]: Opening RCON server socket on TCP/%s", std::to_wstring(port));
	...
	while (true) {
		...
		LOG_DEBUG(L"[RCON]: Waiting for command");
		//get a command from a socket
		int addrLen = sizeof(addr);
		SOCKET remote = accept(listenSock, (sockaddr*)&addr, &addrLen);
		LOG_DEBUG(L"[RCON]: Accepted connection");
		if (remote == INVALID_SOCKET) {
			LOG_ERROR(L"[RCON]: invalid socket error");
			return;
		}
...
unsigned long main_thread(void* lpParameter) {
	...
	LOG_ERROR(L"Chivalry 2 Unchained Plugin");

	LOG_INFO(L"Command line args:");
	...
	if(!loaded) {
		LOG_INFO(L"Continuing with empty build config.");
	}


	baseAddr = GetModuleHandleA("Chivalry2-Win64-Shipping.exe");

	LOG_DEBUG(L"Base address: 0x%p", baseAddr);
	...
	LOG_DEBUG(L"Module base: 0x%p", module_base);
	...
	{
		if (curBuild.offsets.at(StringConverter::toWideString(strFunc[i])) == 0)
			curBuild.offsets.emplace(StringConverter::toWideString(strFunc[i]), FindSignature(baseAddr, moduleInfo.SizeOfImage, strFunc[i], signatures[i]));
		else LOG_INFO(L"ok -> %s : (conf)", strFunc[i]);
	}
	...
	if (curBuild.offsets[StringConverter::toWideString(strFunc[F_UTBLLocalPlayer_Exec])])
	{
		// ServerPlugin
		auto cmd_permission{ module_base + curBuild.offsets[StringConverter::toWideString(strFunc[F_UTBLLocalPlayer_Exec])] }; // Patch for command permission when executing commands (UTBLLocalPlayer::Exec)

		// From ServerPlugin
		// Patch for command permission when executing commands (UTBLLocalPlayer::Exec)
		Ptch_Repl(module_base + curBuild.offsets[StringConverter::toWideString(strFunc[F_UTBLLocalPlayer_Exec])], 0xEB);
	}
	else
		LOG_ERROR(L"F_UTBLLocalPlayer_Exec missing");
	...
	LOG_INFO(L"Functions hooked. Continuing to RCON");
