...
// REGISTER_HOOK_PATCH(GetPakSigningKeysDelegate, APPLY_ALWAYS, void *, ()) {
//   GLOG_INFO("GetPakSigningKeysDelegate called - bypassing EAC pak signing");
//   //   return o_GetPakSigningKeysDelegate();
//   return nullptr;
// }
// REGISTER_BYTE_PATCH(GetPakSigningKeysDelegate, APPLY_ALWAYS,
//                     NO_ADDITIONAL_OFFSET,
//                     (std::vector<uint8_t>{0xB0, 0x01, 0xC3}))

REGISTER_BYTE_PATCH(UTBLLocalPlayer_Exec, APPLY_ALWAYS, NO_ADDITIONAL_OFFSET,
                    {0xEB})

REGISTER_HOOK_PATCH(ExecuteConsoleCommand, APPLY_ALWAYS, void,
                    (FString * param)) {
  GLOG_INFO("Executing console command: {}", std::wstring(param->str));
  o_ExecuteConsoleCommand(param);
}

// REGISTER_HOOK_PATCH(FPakPlatformFile_Mount, APPLY_ALWAYS, bool,
//                     (void /*FPakPlatformFile*/  *this_ptr, wchar_t *param_1,int param_2,wchar_t *param_3,bool param_4)) {
//   GLOG_INFO("FPakPlatformFile__Mount: {}", std::wstring(param_1));
//   return o_FPakPlatformFile_Mount(this_ptr, param_1, param_2, param_3, param_4);
// }

// FText* __cdecl FText::AsCultureInvariant(FText* __return_storage_ptr__,
// FString* param_1)
REGISTER_HOOK_PATCH(FText_AsCultureInvariant, APPLY_ALWAYS, void *,
                    (void *ret_ptr, FString *input)) {
...
// void __thiscall ATBLGameMode::BroadcastLocalizedChat(ATBLGameMode *this,FText
// *param_1,Type param_2)
REGISTER_HOOK_PATCH(BroadcastLocalizedChat, APPLY_ALWAYS, void,
                    (void *game_mode, FText *text, uint8_t chat_type)) {
  GLOG_DEBUG("BroadcastLocalizedChat");
  return o_BroadcastLocalizedChat(game_mode, text, chat_type);
}

bool extractPlayerCommand(const wchar_t *input, std::wstring &playerName,
                          std::wstring &command) {
...
REGISTER_HOOK_PATCH(GetTBLGameMode, APPLY_ALWAYS, void *, (void *uobj)) {
...
APlayerController::ClientMessage
                  (APlayerController *this,FString *param_1,FName param_2,float
param_3)
*/
static bool init = false;
REGISTER_HOOK_PATCH(ClientMessage, APPLY_ALWAYS, void,
                    (void *this_ptr, FString *param_1, void *param_2,
                     float param_3)) {
  ...
  std::string logDir =
      std::string(pValue) + "\\Chivalry 2\\Saved\\Logs\\Unchained";
  ...
  if (out.is_open()) {
    ...
    GLOG_ERROR("Can't open ClientMessage log for writing: {}",
               error_string.has_value() ? error_string.value()
                                        : "Unknown error");
  }
  ...
  if (extractPlayerCommand(param_1->str, playerName, *command)) {
    ...
    FText txt;
    void *res =
        o_FText_AsCultureInvariant(&txt, new FString(L"Command detected"));
