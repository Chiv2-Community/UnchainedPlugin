...
REGISTER_NOP_PATCH(EACAntiCheatMesssage, APPLY_ALWAYS,
                   ADDITIONAL_PLATFORM_OFFSETS(STEAM_OFFSET(0xF)
                                                   EGS_OFFSET(0xE)),
                   5)

REGISTER_HOOK_PATCH(LoadFrontEndMap, APPLY_ALWAYS, bool,
                    (void *this_ptr, FString *param_1)) {
  static wchar_t szBuffer[512];
  static bool init = false;
  if (true) {
    auto pwd_str =
        g_state->GetCLIArgs().server_password.has_value()
    ...
    wsprintfW(szBuffer, L"Frontend%ls%ls%ls",
              ...
              pwd_str.c_str(), init ? L"" : L"?startup");
...
REGISTER_HOOK_PATCH(InternalGetNetMode, APPLY_ALWAYS, ENetMode, (void *world)) {
  g_state->SetUWorld(world);
  return o_InternalGetNetMode(world);
}

REGISTER_HOOK_PATCH(UGameEngineTick,
                    APPLY_WHEN(g_state->GetCLIArgs().rcon_port.has_value()),
                    void,
                    (void *engine, float delta_seconds, uint8_t idle_mode)) {
  // GLOG_TRACE("Engine hook");
  std::optional<std::wstring> next_command =
      g_state->GetRCONState().get_command();
...
REGISTER_HOOK_PATCH(UNetDriver_GetNetMode,
...
REGISTER_HOOK_PATCH(UGameplay_IsDedicatedServer, APPLY_ALWAYS, bool,
                    (long long param_1)) {
  if (g_state->GetUWorld() != nullptr &&
      !g_state->GetCLIArgs().playable_listen) {
